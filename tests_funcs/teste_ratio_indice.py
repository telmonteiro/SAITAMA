...
def line_ratio_indice(data, line="CaI"):
    ...
    for i in range(len(data)):
        ...
        flux_continuum = np.median([flux_left,flux_right]) #mean
...
#first spectrum
wv, flux, hdr = read_fits(files[0],instrument=instr,mode="rv_corrected")
plt.figure(1)
ratio_arr, center_flux_line_arr, flux_continuum_arr = line_ratio_indice([(wv,flux)], line="CaI")
print("Ratio: ",ratio_arr[0])
print("Flux center line: ",center_flux_line_arr[0])
...
plt.axvline(x=0.02,ymin=0,ymax=1,ls="--",ms=0.1)
plt.axvline(x=-0.02,ymin=0,ymax=1,ls="--",ms=0.1)
...
'''
vai as "asas", saca os primeiros e ultimos 10 pontos e faz a mediana de cada. depois faz a media dos dois e esse é o fluxo continuo

vai a zona da linha tabelada e num intervalo de 0.06 angs vê os valores que estao nesse intervalo e faz a mediana (em principio calha no centro da linha ou mt proximo) e 
esse é o fluxo do centro da linha

rácio entre os dois para grande intervalo de offsets. vai aparecer um plot ondulado em que o minimo rácio é 0 offset. portanto se o espetro estiver no wv errado o minimo
nao vai ser no 0 offset (com intervalo de tolerancia, tipo 0.02)

o indice a guardar seria entao uma flag apenas?

posso correr para vários espetros e guardar o rácio no 0 offset, se desviar muito é porque está mal
